            ...
            <table class="datatable datatable-bordered datatable-head-custom" id="kt_datatable">
                ...
                @foreach($data as $row)
                    <tr>
                        ...
                        <td>
                            <a href="{{route('subjects.show',$row->id)}}" class="btn btn-dark mr-2">{{trans('s_admin.nav_subjects')}}</a>
                        </td>
                        <td class="text-right">
                            ...
                            <form method="get" id='delete-form-{{ $row->id }}' action="{{url('levels/'.$row->id.'/delete')}}" style='display: none;'>
                            {{csrf_field()}}
                            <!-- {{method_field('delete')}} -->
                            </form>
